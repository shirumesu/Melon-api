...
export class BangumiClient {
  ...
  async getSubject(subjectId: number): Promise<SubjectListItem> {
    const subject = await this.fetchJson<BangumiSubject>(
  ...
  async getEpisode(episodeId: number): Promise<Episode> {
    const episode = await this.fetchJson<BangumiEpisode>(
      `/v0/episodes/${episodeId}`,
    );
  ...
  async getCharacters(subjectId: number): Promise<CharacterCredit[]> {
    ...
    return response.map((character) => ({
      ...
      actors: (character.actors ?? []).map((actor) =>
        this.mapPerson(actor, actor.relation),
      ),
  ...
  async getPersons(subjectId: number): Promise<StaffCredit[]> {
    const response = await this.fetchJson<RelatedPersonResponse[]>(
  ...
  async getRelatedSubjects(subjectId: number): Promise<RelatedSubject[]> {
    const response = await this.fetchJson<RelatedSubjectResponse[]>(
  ...
  mapEpisode(episode: BangumiEpisode, fallbackSubjectId: number): Episode {
    const sort =
      typeof episode.ep === "number" && episode.ep > 0
        ? episode.ep
        : episode.sort;
    return {
...
export function mapInfobox(
  infobox: BangumiSubject["infobox"],
): SubjectInfoBoxItem[] {
...
function infoboxValueToString(value: InfoboxResponseItem["value"]): string {
  ...
  return value
    .map((entry) =>
      "k" in entry && entry.k ? `${entry.k}: ${entry.v}` : entry.v,
    )
    .filter(Boolean)
    .join(" / ");
}
...
export function subjectIdFromSites(
  sites: Array<{ site?: string; id?: string | number | null }>,
): number | undefined {
