...
import { fetchEpisodeComments, fetchSubjectComments, fetchSubjectTopics } from "./html";
  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      if (typeof maybeHttp.status === "number") {
        return errorJson(maybeHttp.status, maybeHttp.code, maybeHttp.message, maybeHttp.details);
      }
      return errorJson(500, "INTERNAL_ERROR", error instanceof Error ? error.message : "Unknown error");
    }
  },
...
async function route(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  if (request.method === "GET" && /^v1\/subjects\/\d+\/characters$/.test(path)) {
  ...
  if (request.method === "GET" && path === "v1/schedule/latest") return getSchedule(url, env);
  if (request.method === "GET" && path === "v1/schedule/today") return getTodaySchedule(url, env);
  if (request.method === "GET" && path === "v1/seasons/current") return getSeason(url, env, "current");
  if (request.method === "GET" && path === "v1/trending/current") return getSeason(url, env, "trending");
  if (request.method === "POST" && path === "v1/internal/refresh") {
    const rejected = requireAdmin(request, env);
    if (rejected) return rejected;
    ctx.waitUntil(refreshMaterializedCaches(env));
    return json({ accepted: true, startedAt: new Date().toISOString() }, { headers: { "cache-control": "no-store" } });
  }
...
async function searchSubjects(url: URL, env: Env): Promise<Response> {
  ...
  const result = await getOrSetJson(env, key, { ttlSeconds: 30 * 60, force }, () =>
    client.searchSubjects(input)
  );
  return json({ ...result.value, cache: result.cache });
}

async function getSubject(subjectId: number, url: URL, env: Env): Promise<Response> {
  const full = !["0", "false"].includes(url.searchParams.get("full") ?? "");
  const force = boolParam(url.searchParams.get("force"));
  const key = cacheKey(["subjects", subjectId, full ? "full" : "brief"]);
  const result = await getOrSetJson(env, key, { ttlSeconds: full ? 6 * 60 * 60 : 60 * 60, force }, async () => {
    const client = new BangumiClient(env);
    if (!full) return client.getSubject(subjectId);

    const subject = await client.getSubjectRaw(subjectId);
    const notes: string[] = [];
    const [episodes, characters, staff, relatedSubjects, comments, topics, schedule] =
      await Promise.all([
    ...
    detail.schedule = schedule ? scheduleForSubject(schedule.items, subjectId) ?? fallbackScheduleFromAirDate(detail.airDate) : fallbackScheduleFromAirDate(detail.airDate);
    return detail;
  });
  return json({ data: result.value, cache: result.cache });
}

async function getSubjectEpisodes(subjectId: number, url: URL, env: Env): Promise<Response> {
  const force = boolParam(url.searchParams.get("force"));
  const client = new BangumiClient(env);
  const result = await getOrSetJson(env, cacheKey(["subjects", subjectId, "episodes"]), { ttlSeconds: 6 * 60 * 60, force }, () =>
    client.getEpisodes(subjectId)
  );
  return json({ data: result.value, cache: result.cache });
}

async function getSubjectCharacters(subjectId: number, url: URL, env: Env): Promise<Response> {
  const force = boolParam(url.searchParams.get("force"));
  const client = new BangumiClient(env);
  const result = await getOrSetJson(env, cacheKey(["subjects", subjectId, "characters"]), { ttlSeconds: 24 * 60 * 60, force }, () =>
    client.getCharacters(subjectId)
  );
  return json({ data: result.value, cache: result.cache });
}

async function getSubjectStaff(subjectId: number, url: URL, env: Env): Promise<Response> {
  const force = boolParam(url.searchParams.get("force"));
  const client = new BangumiClient(env);
  const result = await getOrSetJson(env, cacheKey(["subjects", subjectId, "staff"]), { ttlSeconds: 24 * 60 * 60, force }, () =>
    client.getPersons(subjectId)
  );
  return json({ data: result.value, cache: result.cache });
}

async function getSubjectComments(subjectId: number, url: URL, env: Env): Promise<Response> {
  const force = boolParam(url.searchParams.get("force"));
  const result = await getOrSetJson(env, cacheKey(["subjects", subjectId, "comments"]), { ttlSeconds: 30 * 60, force }, () =>
...
async function getSubjectTopics(subjectId: number, url: URL, env: Env): Promise<Response> {
  const force = boolParam(url.searchParams.get("force"));
  const result = await getOrSetJson(env, cacheKey(["subjects", subjectId, "topics"]), { ttlSeconds: 30 * 60, force }, () =>
...
async function getEpisode(episodeId: number, url: URL, env: Env): Promise<Response> {
  const force = boolParam(url.searchParams.get("force"));
  const client = new BangumiClient(env);
  const result = await getOrSetJson(env, cacheKey(["episodes", episodeId]), { ttlSeconds: 6 * 60 * 60, force }, () =>
    client.getEpisode(episodeId)
  );
  return json({ data: result.value, cache: result.cache });
}

async function getEpisodeComments(episodeId: number, url: URL, env: Env): Promise<Response> {
  const force = boolParam(url.searchParams.get("force"));
  const result = await getOrSetJson(env, cacheKey(["episodes", episodeId, "comments"]), { ttlSeconds: 30 * 60, force }, () =>
...
async function getSeason(url: URL, env: Env, mode: "current" | "trending"): Promise<Response> {
  ...
  const input: SearchInput = {
    ...
    sort: mode === "trending" ? "heat" : normalizeSort(url.searchParams.get("sort")) ?? "rank",
    tags: readListParam(url, "tag"),
    metaTags: readListParam(url, "metaTag"),
    airDates: readListParam(url, "airDate").concat([`>=${range.start}`, `<=${range.end}`]),
...
function normalizeSort(value: string | null): SearchInput["sort"] | undefined {
  if (value === "match" || value === "heat" || value === "rank" || value === "score") return value;
  return undefined;
}
