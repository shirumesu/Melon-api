...
export function readListParam(url: URL, name: string): string[] {
  ...
  const jsonValue = url.searchParams.get(name.endsWith("s") ? name : `${name}s`);
...
export function seasonDateRange(season: SeasonInfo): { start: string; end: string } {
...
export function parseSeason(value: string | null): SeasonInfo {
  ...
  if (match) {
    ...
    const names = ["WINTER", "SPRING", "SUMMER", "FALL"] as const;
    const name = names[quarter - 1] ?? "WINTER";
    return { year, quarter, code: `${year}Q${quarter}`, label: `${year} ${name}`, name };
  }
  const labelMatch = /^(\d{4})\s*(WINTER|SPRING|SUMMER|FALL)$/.exec(normalized);
  if (labelMatch) {
    const year = Number(labelMatch[1]);
    const names = ["WINTER", "SPRING", "SUMMER", "FALL"] as const;
    const quarter = (names.indexOf(labelMatch[2] as SeasonInfo["name"]) + 1) as 1 | 2 | 3 | 4;
    return { year, quarter, code: `${year}Q${quarter}`, label: `${year} ${labelMatch[2]}`, name: labelMatch[2] as SeasonInfo["name"] };
  }
